...
The aim of this laboratory was to implement an open-loop camera stabilization system on the TurtleBot using data from its IMU and controlling two servo motors to adjust the tilt and the pan of the camera. 
    ...
    In order to get a 100Hz update frequency, we have the following timer configuration in place:
    \begin{enumerate}
        \item \textbf{Prescaler}: The prescaler is used to divide the input clock frequency of 96 MHz by 9600 in order to set the timer's counting frequency to 10kHz;
...
\section{Laboratory 4}
        ...
        As shown in the picture, the error that the yaw controller is receiving is made from the difference between the reference and the current error. We want that the error to be minimal.
        It is also possible to rewrite the equation stated above with respect to the error of followin the line, namely ${e_{esl}}$.
    ...
    Therefore, using basic mathematical knowledge, the arcatan(x) is not needed when the value of x is close to 0 (zero). In conclusion, we can define the yaw error as follows:
    \begin{equation}
        \psi_{err} = \frac{e_{esl}}{H};
    \end{equation}

    We implemented the yaw erorr by as follows:
  ...
  trurtle_bot->TB_jaw_error = (line_attributes->distance_from_center/LINE_SENSOR_DISTANCE);
    ...
    \begin{figure}[H]
		...
		\end{figure}
    For the controller, namely $C_{\psi}$, is multiplied by the error that we have defined above. Taken this into consideration, we will define the controller of the yaw by implementing a proportional (P) control. The value of the yaw controller is defined by the variable $K_r$.
    ...
    With everythind defined, we can introduce the Simulink model implemented by us:
    ...
    We can clearly see both the PID-s that are intended for controlling both of the wheels speeds. In the efforts of finding the suitable PID for both of the wheels such that the overall error is minimal, I implemented a metaheuristic method using the Matlab implemented function, namely PSO.
    PSO stands for Particle Swarm Optimization and it is a method in which we can find the parameters of both the PID-s and the yaw controller gain by running the simulation multiple times and placing particles all over the search space.

    The particles have unique attributes, such as :
    \begin{enumerate}
        \item they have knowledge of their position in the search space
        \item they have knowledge of the swarms position in the search space
        \item they have knowledge of their value
        \item they have knowledge of the swarms values
    \end{enumerate}
    ...
    In this way, the goal of the algorithm is to move the particles in the search space at every iteration, then the particles, by running the simulation with the values obtained in that iteration, obtain an output that is being normalized for easier comparison. In the end, after multiple iterations, the particles should gather in a small area which in the end is the global maximum value, namely the minimal error found in the designated search space.

    For the algorithm to work properly, I have implemented different intervals that the values should take. Therefore, we can run the same algorithm, but with multiple choices, building multiple configurations.
...
\item $param_{ranges} = [
    ...
    As we can see, the output perfectly tracks the the step reference with these values. Taking this into account, we call it a success in tracking the step reference. Although, by testing these values on the bot, we could not get the results that we were hoping for. I think this is to the fact that the reference is different or the sensor does not read as the simulation intends.
